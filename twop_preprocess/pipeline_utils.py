...
def sbatch_session(project, session_name, pipeline_filename, conflicts, tau=0.7):
    ...
    script_path = str(Path(__file__).parent.parent / pipeline_filename)

    log_fname = f"{session_name}_%j.log"

    log_path = str(Path(__file__).parent.parent / "logs" / f"{log_fname}")
